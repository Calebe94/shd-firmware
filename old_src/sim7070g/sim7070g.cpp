...
void sim7070g_setup(void)
{
    ...
    uart_config.baud_rate = SIM7070G_BAUD,
    ...
    uart_driver_install(SIM7070G_PORT, 127*2, 0, 0, NULL, 0);
    uart_param_config(SIM7070G_PORT, &uart_config);

    uart_set_pin(SIM7070G_PORT, SIM7070G_TXD, SIM7070G_RXD, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
...
void sim7070g_init(void)
{
    ...
    io_conf.mode = (gpio_mode_t)(GPIO_MODE_OUTPUT|GPIO_MODE_INPUT);
    ...
    gpio_isr_handler_add((gpio_num_t)SIM7070G_INT, sim7070g_interruption_handler, (void*)SIM7070G_LED);

    gpio_set_level((gpio_num_t)SIM7070G_LED, 0);
    gpio_set_level((gpio_num_t)SIM7070G_POWER, 1);

    gpio_set_level((gpio_num_t)SIM7070G_PWR, 1);
    vTaskDelay(pdMS_TO_TICKS(500));
    gpio_set_level((gpio_num_t)SIM7070G_PWR, 0);
...
void sim7070g_send_sms(const char *number, const char *message)
{
    ...
    length = uart_read_bytes((uart_port_t)1, data, length, 100);
        ...
        if(xQueueReceive(sim7070g_queue, (void * )&event, (portTickType)portMAX_DELAY)) {
            ...
            switch(event.type) {
                ...
                case UART_DATA:
                    ...
                    uart_read_bytes((uart_port_t)SIM7070G_PORT, dtmp, event.size, portMAX_DELAY);
                ...
                case UART_FIFO_OVF:
                    ...
                    uart_flush_input((uart_port_t)SIM7070G_PORT);
                ...
                case UART_PATTERN_DET:
                    {
                    uart_get_buffered_data_len((uart_port_t)SIM7070G_PORT, &buffered_size);
                    int pos = uart_pattern_pop_pos((uart_port_t)SIM7070G_PORT);
                    ...
                    {
                        uart_read_bytes((uart_port_t)SIM7070G_PORT, dtmp, pos, 100 / portTICK_PERIOD_MS);
                        uint8_t pat[1 + 1];
                        memset(pat, 0, sizeof(pat));
                        uart_read_bytes((uart_port_t)SIM7070G_PORT, pat, 1, 100 / portTICK_PERIOD_MS);
...
bool sim7070g_turn_modem_on(void)
{
    ...
    while(tries--)
    {
        ...
        uint8_t data[128];
        int length = 0;
        ESP_ERROR_CHECK(uart_get_buffered_data_len((uart_port_t)1, (size_t*)&length));
        length = uart_read_bytes((uart_port_t)1, data, length, 100);
...
{
    size_t length = 0;
    ESP_ERROR_CHECK(uart_get_buffered_data_len((uart_port_t)SIM7070G_PORT, &length));
    length = uart_read_bytes((uart_port_t)SIM7070G_PORT, (uint8_t*)data, length, 100);
    return length;
}

void sim7070g_flush(void)
{
    uart_flush((uart_port_t)SIM7070G_PORT);
    uart_flush_input((uart_port_t)SIM7070G_PORT);
}
